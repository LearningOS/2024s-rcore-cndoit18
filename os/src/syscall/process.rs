...
use crate::{
    ...
    task::{
        ...
        get_current_task_info, suspend_current_and_run_next, TaskControlBlock, TaskStatus,
    },
...
pub fn sys_mmap(start: usize, len: usize, port: usize) -> isize {
    ...
    if tcb.memory_set.is_conflicts(start_va, end_va) {
        return -1;
    }
...
pub fn sys_munmap(start: usize, len: usize) -> isize {
    ...
    let (start_va, end_va) = (VirtAddr::from(start), VirtAddr::from(start + len));
    if !start_va.aligned() || start_va >= end_va {
        return -1;
    }
    let task = current_task().unwrap();
    let mut tcb = task.inner_exclusive_access();
    if tcb.memory_set.remove_area(start_va.floor(), end_va.ceil()) {
        return 0;
    }
    -1
}
